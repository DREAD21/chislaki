...
if __name__ == '__main__':
    file = open("in13.txt", "r")
    data = np.genfromtxt('in13.TXT', delimiter=',')
    n = data.shape[0]
    b = [0] * data.shape[0]
    a = [[0 for j in range(data.shape[0])] for i in range(data.shape[0])]
    ...
    print('x=', simpleIteration(a, n, b))
    print('Решение по методу Зейделя:')
    print('x=', seidel(a, b, 0.01))
